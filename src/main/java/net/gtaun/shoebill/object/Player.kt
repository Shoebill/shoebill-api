...
abstract class Player : Proxyable<Player> {
     ...
     */
    abstract val stuntBonusEnabled: Boolean

    /**
     * If the [Player] is spectating.
     */
    abstract val spectating: Boolean

    /**
     * Iff the [Player] is recording.
     */
     ...
     */
    abstract val controllable: Boolean
    ...
    /**
     * Sets the [Velocity] of the [Player] to [vel].
     * @param vel New Velocity
     */
    abstract fun setVelocity(vel: Velocity)
    ...
    companion object : Collectable<Player>, Findable<Int, Player> {
        ...
        fun getByNameOrId(nameOrId: String): Player? {
            var nameOrIdTrimmed = nameOrId.trim { it <= ' ' }
        ...
        fun getByPartOfName(partOfName: String): Player? {
            var partOfNameTrimmed = partOfName.trim { it <= ' ' }
            var highestHit = -25
